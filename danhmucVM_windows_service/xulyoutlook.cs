...
namespace danhmucVM_windows_service
{
    class xulyoutlook
    {
        static string duongdangoc = AppDomain.CurrentDomain.BaseDirectory;
        private static xulyoutlook _instance = null;
        public static xulyoutlook Instance()
        {
            if (_instance == null)
                _instance = new xulyoutlook();
            return _instance;
        }
        private string _tenfile = null;
                                        ...
                                        if (extensionsArray.Any(fn.Contains))
                                        {
                                            if (Regex.IsMatch(mi.Attachments[i].FileName, mau))
                                            {
                                                ...
                                                if (!File.Exists(duongdangoc + @"\filedanhmuc\" + mi.Attachments[i].FileName))
                                                {
                                                    ...
                                                    _tenfile = mi.Attachments[i].FileName;
                                                }
                                                else
                                                {
